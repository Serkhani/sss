  ...
  const NavigationContent = () => (
    <div className="space-y-2">
      ...
      <NavButton
        ...
        onClick={() => { setActiveTab('publish'); setIsMobileMenuOpen(false); }}
      ...
      <NavButton
        ...
        onClick={() => { setActiveTab('bot'); setIsMobileMenuOpen(false); }}
      ...
      <NavButton
        ...
        onClick={() => { setActiveTab('simulate'); setIsMobileMenuOpen(false); }}
        icon={<Activity className="w-4 h-4" />}
      ...
      <NavButton
        ...
        onClick={() => { setActiveTab('combined'); setIsMobileMenuOpen(false); }}
        icon={<Activity className="w-4 h-4" />}
      ...
      <NavButton
        ...
        onClick={() => { setActiveTab('access-control'); setIsMobileMenuOpen(false); }}
        icon={<Radio className="w-4 h-4" />}
      ...
      <NavButton
        ...
        onClick={() => { setActiveTab('reader'); setIsMobileMenuOpen(false); }}
        icon={<Radio className="w-4 h-4" />}
        label="Reader"
    ...
    <ToastProvider>
      <StreamProvider>
        <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#0B0C15] to-[#0B0C15] text-slate-200 font-sans selection:bg-indigo-500/30">
          ...
          {/* Header */}
          <header className="border-b border-slate-800/60 sticky top-0 z-30 backdrop-blur-md bg-[#0B0C15]/70">
            <div className="container mx-auto px-4 h-16 flex items-center justify-between">
              <div className="flex items-center gap-3">
                {/* Mobile Menu Button */}
                <button
                  onClick={() => setIsMobileMenuOpen(true)}
                  className="lg:hidden p-2 -ml-2 text-slate-400 hover:text-white rounded-lg hover:bg-slate-800/50"
                >
                ...
                <div>
                  <h1 className="text-lg font-bold tracking-tight text-white hidden sm:block">Somnia Stream Studio</h1>
                  <h1 className="text-lg font-bold tracking-tight text-white sm:hidden">SSS</h1>
                  <p className="text-[10px] text-slate-400 font-medium tracking-wider uppercase hidden sm:block">Dream Computer Interface</p>
                </div>
              </div>
